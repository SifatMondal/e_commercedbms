    ...
    try {
        const result = await pool.query(
            `SELECT COUNT(1)::int AS unread_count
             FROM notifications
             WHERE ${column} = $1 AND status != 'Read'`,
            [req.user.sub]
        );
...
exports.markNotificationRead = async (req, res) => {
    ...
    try {
        ...
        eventService.broadcast("notification_read", { recipientRole: req.user.role, recipientId: req.user.sub, notificationId });
...
exports.markAllRead = async (req, res) => {
    ...
    try {
        ...
        eventService.broadcast("notification_read", { recipientRole: req.user.role, recipientId: req.user.sub });
