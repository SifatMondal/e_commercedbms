...
async function findProduct(client, productId) {
    const result = await client.query(
        "SELECT product_id, stock, status FROM products WHERE product_id = $1 FOR SHARE",
        [productId]
    );
    return result.rows[0];
}
...
exports.addCartItem = async (req, res) => {
    ...
    try {
        ...
        if (product.status === "paused") {
            await client.query("ROLLBACK");
            return res.status(400).json({ message: "This product is currently paused and cannot be added to the cart." });
        }
