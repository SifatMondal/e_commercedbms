...
const { createCustomerNotification, createSellerNotification } = require("../services/notificationService");
...
exports.respondToRequest = async (req, res) => {
  ...
  try {
    ...
    await client.query("COMMIT"); return res.json({ message: "Delivery request accepted and order assigned." });
...
exports.setAvailability = async (req, res) => {
  ...
  try { if (status === "available") { const active = await pool.query("SELECT 1 FROM orders WHERE deliveryman_id=$1 AND delivery_status = ANY($2::VARCHAR[]) LIMIT 1", [req.user.sub, ACTIVE]); if (active.rowCount) return res.status(409).json({ message: "Complete active deliveries before becoming available." }); }
    const result = await pool.query("UPDATE deliverymen SET availability_status=$1 WHERE deliveryman_id=$2 RETURNING availability_status", [status, req.user.sub]); return res.json({ message: "Availability updated.", availability_status: result.rows[0].availability_status });
...
exports.updateDelivery = async (req, res) => {
  ...
  const client = await pool.connect(); try {
    ...
    await client.query("COMMIT"); return res.json({ message: "Delivery updated.", order: updated.rows[0] });
